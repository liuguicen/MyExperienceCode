...
import person.lgc.myexperiencecode.R;
import person.lgc.myexperiencecode.bitmapTool.AsyncImageLoader3;
...
public class GridViewAdapter extends BaseAdapter {
     ...
     */
    public GridViewAdapter(Context context, List<String> imagUrls) {
        mContext = context;
...
// android:layout_height="fill_parent"
        mLinearLayout.setLayoutParams(new GridView.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, 333));
...
// android:orientation="vertical"
    ...
    ImageView creatItemImage(LinearLayout linearLayout) {
        ...
        LinearLayout.LayoutParams mLayoutParams = new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, 333);
    ...
    public View getView(int position, View holder, ViewGroup parent) {
        ...
        if (cachedImage == null && position <= STATIC_SHOW_NUMBER) {
            imageLoader.loadBitmap(path, setter.ivImage, position, imageCallback, 333);
        }
        if (cachedImage != null && setter.ivImage != null) {
            setter.ivImage.setImageBitmap(cachedImage);
        } else if (setter.ivImage != null) {
            setter.ivImage.setImageResource(R.mipmap.ic_launcher);
        }
